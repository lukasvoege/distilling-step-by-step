...
from metrics import (
    compute_text_acc,
    compute_equation_acc,
    compute_metrics_text,
    compute_metrics_equation,
    compute_metrics_text_aux,
    compute_metrics_equation_aux,
)
...
def run(args):
    ...
    if args.llm is None:
        ...
        train_llm_rationales, train_llm_labels = dataset_loader.load_gpt35_preds(
            split="train", prompt_mix_id=args.prompt_mix
        )
        test_llm_rationales, test_llm_labels = dataset_loader.load_gpt35_preds(
            split="test", prompt_mix_id=args.prompt_mix
        )
    else:
        raise ValueError
    ...
    if dataset_loader.has_valid:
        if args.llm is None:
            ...
            valid_llm_rationales, valid_llm_labels = dataset_loader.load_gpt35_preds(
                split="valid", prompt_mix_id=args.prompt_mix
            )
        else:
            raise ValueError
    ...
    if args.model_type == "task_prefix" and args.llm is not None:
        ...
        def tokenize_function(examples):
            model_inputs = tokenizer(
                ["predict: " + text for text in examples["input"]], max_length=args.max_input_length, truncation=True
            )
            expl_model_inputs = tokenizer(
                ["explain: " + text for text in examples["input"]], max_length=args.max_input_length, truncation=True
            )
    ...
    if args.llm is None:
        tokenized_datasets = datasets.map(tokenize_function, remove_columns=["input", "label"], batched=True)
    else:
        tokenized_datasets = datasets.map(
            tokenize_function, remove_columns=["input", "rationale", "label", "llm_label"], batched=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_save", action="store_true")    ###### TEST!!
    ...
    parser.add_argument("--es_threshold", type=float, default=0.005)
