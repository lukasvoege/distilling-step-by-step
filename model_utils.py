...
class TaskPrefixDataCollator(DataCollatorForSeq2Seq):
    def __call__(self, features, return_tensors=None):
        ...
        pred_features = features_df.loc[
            :, ~features_df.columns.isin(["aux_labels", "expl_input_ids", "expl_attention_mask"])
        ].to_dict("records")
        expl_features = (
            features_df.loc[:, ~features_df.columns.isin(["labels", "input_ids", "attention_mask"])]
            .rename(
...
class TaskPrefixTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss = self.alpha * pred_outputs.loss + (1.0 - self.alpha) * expl_outputs.loss
    ...
    def prediction_step(
        ...
        pred_outputs = super().prediction_step(
            model, inputs["pred"], prediction_loss_only=False, ignore_keys=ignore_keys
        )
        if self.output_rationale:
            expl_outputs = super().prediction_step(
                model, inputs["expl"], prediction_loss_only=False, ignore_keys=ignore_keys
            )
        else:
